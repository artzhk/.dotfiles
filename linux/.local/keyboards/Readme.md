...
1. Put the archive to `/usr/share/kbd/keymaps` and set the `/etc/vconsole.conf`  
...
In my case it is `evdev`

3. Open to the `/usr/share/X11/xkb/rules/$(name_from_the_output).xml`  
Paste the following to in US section close to other dvoraks
```xml
        <variant>
          <configItem>
            <name>real-programmer-dvorak</name>
...
4. Navigate to the `/usr/share/X11/xkb/symbols/` open us and paste the  
